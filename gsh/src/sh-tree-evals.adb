...
package body Sh.Tree.Evals is
   ...
   begin
      ...
      declare
         Result_String : CList;
      begin
         Eval_String (S, Result_String, Tokens.As_String (N.Cmd));
         Eval_String_List (S, Result_String, T, N.Arguments);
         ...
         declare
            Cmd         : constant String := Element (Result_String, 1);
   ...
   begin
      if not N.Loop_Default_Values then
         Eval_String_List (S, Loop_Var_Values, T, N.Loop_Var_Values);
      else
         Eval_String (S, Loop_Var_Values, """$@""");
      end if;
      ...
      if Length (Loop_Var_Values) > 0 then
         Set_Loop_Scope_Level (S, My_Nested_Level);
         for Idx in 1 .. Length (Loop_Var_Values) loop
            if not Callback (Element (Loop_Var_Values, Idx)) then
               exit;
            end if;
         end loop;
         Deallocate (Loop_Var_Values);
      ...
      begin
         ...
         then
            declare
               Trap_Arg : CList;
            begin
               Append (Trap_Arg, Exit_Trap_Action.all);
