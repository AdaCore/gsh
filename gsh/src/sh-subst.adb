...
package body Sh.Subst is
   ...
   procedure Simple_Filename_Expansion
   ...
   is
      ...
      A_Result          : Annotated_String := Eval_String_Aux
        (SS, S, Characters_Read, Has_Command_Subst => Has_Command_Subst);
   begin
      Split_String (SS, Result, A_Result, Max_Split);
   end Eval_String;
   ...
   begin
      Append (AS, S);
      Split_String (SS, Result, AS, Max_Split);
   end Split_String;
   ...
   is
      ...
      begin
         ...
         begin
            Eval_String (SS, Arith_List, Delimited_Expression);
            ...
            for Idx in 1 .. Length (Arith_List) loop
               Split_Arithmetic_String (SS,
                                        Element (Arith_List, Idx),
                                        Previous_Was_Number,
                                        Tokenized_List);
            end loop;
            Append (Buffer,
                    Eval_Expr (SS, As_List (Tokenized_List), True));
   ...
   is
      ...
      Prefix  : constant String := Format_Pathname (Dir, UNIX);
   ...
   begin
      ...
      begin
         if Dir = "" then
            D := OS.FS.Dir.Open (Normalize_Path (State, "."));
         else
            D := OS.FS.Dir.Open (Normalize_Path (State, Dir));
         end if;
      exception
         when others =>
            return;
